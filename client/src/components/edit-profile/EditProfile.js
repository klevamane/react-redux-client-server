// We copied everything from the create profile component
// we used it as a base and made edits
...
// note that we are getting the profile from the profile object
        if(nextProps.profile.profile) {
            const profile = nextProps.profile.profile;

            // Bring skills array back to comma seperated value
            // Turns the array into a string and seperate each value with a comma
            const skillsCSV = profile.skills.join(',');

            // if the profile field doesn't exist, make the string empty
            profile.company = !isEmpty(profile.company) ? profile.company : '';
            profile.website = !isEmpty(profile.website) ? profile.website : '';
            profile.location = !isEmpty(profile.location) ? profile.location : '';
            profile.githubusername = !isEmpty(profile.githubusername) ? profile.githubusername : '';
            profile.bio = !isEmpty(profile.bio) ? profile.bio : '';
            profile.social = !isEmpty(profile.social) ? profile.social : {}; //  because social is an object
